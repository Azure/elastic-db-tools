...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement
{
    ...
    public sealed class ShardKey : IComparable<ShardKey>, IEquatable<ShardKey>
    {
        ...
        private static readonly Dictionary<Type, ShardKeyType> TypeToShardKeyType = new Dictionary<Type, ShardKeyType>()
            {
        ...
        private static readonly Dictionary<ShardKeyType, Type> ShardKeyTypeToType = new Dictionary<ShardKeyType, Type>()
            {
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Security", "CA2104:DoNotDeclareReadOnlyMutableReferenceTypes",
            Justification = "ShardKey type is immutable.")]
        public static readonly ShardKey MinBinary = new ShardKey(ShardKeyType.Binary, ShardKey.EmptyArray);
        ...
        private readonly ShardKeyType keyType;
        ...
        public ShardKey(object value)
        {
            ...
            {
                this.keyType = ShardKey.DetectShardKeyType(value);
        ...
        private ShardKey(ShardKeyType keyType, byte[] rawValue, bool validate)
        {
            ...
            if (validate)
            {
                ...
                if (this.value == null ||
                    this.value.Length == 0 ||
                    this.value.Length == expectedLength)
                {
                    return;
                }
                ...
                if (this.keyType != ShardKeyType.Binary || this.value.Length > expectedLength)
                {
                    throw new ArgumentOutOfRangeException(
                        "rawValue",
                        rawValue,
        ...
        public byte[] RawValue
        {
            get
            {
                ...
                {
                    byte[] rawValueCpy = new byte[this.value.Length];
                    this.value.CopyTo(rawValueCpy, 0);
                    return rawValueCpy;
                }
            }
        }
        ...
        public object Value
        {
            get
            {
                return ShardKey.DeNormalize(this.keyType, this.value);
            }
        }
        ...
        public Type DataType
        {
            get
            {
                return ShardKey.ShardKeyTypeToType[this.keyType];
            }
        }
        ...
        public override string ToString()
        {
            ...
            else
            {
                switch (this.keyType)
                {
                    ...
                    case ShardKeyType.Binary:
                        return StringUtils.ByteArrayToString(this.value);
                    default:
                        Debug.Assert(this.keyType == ShardKeyType.None);
        ...
        public int CompareTo(ShardKey other)
        {
            ...
            if (KeyType == ShardKeyType.DateTimeOffset)
            {
                ...
                Buffer.BlockCopy(value, 0, rawThisValue, 0, Buffer.ByteLength(rawThisValue));
                Buffer.BlockCopy(other.value, 0, rawOtherValue, 0, Buffer.ByteLength(rawOtherValue));
            ...
            Int32 minLength = Math.Min(this.value.Length, other.value.Length);
            ...
            else
            {
                ...
                return this.value[differentByteIndex].CompareTo(other.value[differentByteIndex]);
            }
        }
        ...
        public static ShardKeyType DetectShardKeyType(object value)
        {
            ...
            if (!ShardKey.TypeToShardKeyType.TryGetValue(value.GetType(), out keyType))
            {
                throw new ArgumentException(
                    StringUtils.FormatInvariant(
        ...
        {
            return TypeToShardKeyType.ContainsKey(type);
        }
        ...
        public static Type TypeFromShardKeyType(ShardKeyType keyType)
        {
            ...
            return ShardKeyTypeToType[keyType];
        }
        ...
        {
            if (TypeToShardKeyType.ContainsKey(type))
            {
                return TypeToShardKeyType[type];
            }
            else
            {
                throw new ArgumentOutOfRangeException(
            ...
            else
            {
                ...
                switch (this.KeyType)
                {
                    ...
                    case ShardKeyType.DateTimeOffset:
                        ...
                        Buffer.BlockCopy(value, Buffer.ByteLength(interimNextKey.RawValue), bRes, Buffer.ByteLength(interimNextKey.RawValue), sizeof(long));
        ...
        {
            int hash = (int)this.keyType * 3137;

            if (null != this.value)
            {
                byte[] tempArray = null;
                if (KeyType == ShardKeyType.DateTimeOffset && value.Length == SizeOfDateTimeOffset)
                {
                    tempArray = new byte[sizeof(long)];
                    Buffer.BlockCopy(value, 0, tempArray, 0, Buffer.ByteLength(tempArray));
                }
                else
                {
                    tempArray = new byte[this.value.Length];
                    this.value.CopyTo(tempArray, 0);
                }
