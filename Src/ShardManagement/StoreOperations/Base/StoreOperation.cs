    ...
    internal abstract class StoreOperation : IStoreOperation
    {
        ...
        public IStoreResults Do()
        {
                ...
                do
                {
                    result = this.Manager.RetryPolicy.ExecuteAction(() =>
                    {
                        ...
                        finally
                        {
                            ...
                            if (this.maxDoState < this.operationState)
                            {
            ...
            catch (StoreException se)
            {
                ...
                throw this.OnStoreException(se, this.operationState);
            }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                this.localConnectionTarget.Dispose();
                this.localConnectionTarget = null;
            }

            if (this.localConnectionSource != null)
            {
                this.localConnectionSource.Dispose();
        ...
        private void AttemptUndo()
        {
            ...
            this.UndoStartState = UndoStateForDoState(this.maxDoState);
        ...
        {
            this.operationState = undo ? StoreOperationState.UndoGlobalConnect : StoreOperationState.DoGlobalConnect;
            ...
            this.globalConnection = this.Manager.StoreConnectionFactory.GetConnection(
                StoreConnectionKind.Global,
                this.Manager.Credentials.ConnectionStringShardMapManager);

            this.globalConnection.OpenWithLock(this.Id);

            if (sci.SourceLocation != null)
            {
                this.operationState = undo ? StoreOperationState.UndoLocalSourceConnect : StoreOperationState.DoLocalSourceConnect;

                this.localConnectionSource = this.Manager.StoreConnectionFactory.GetConnection(
                    StoreConnectionKind.LocalSource,
                    this.GetConnectionStringForShardLocation(sci.SourceLocation));

                this.localConnectionSource.OpenWithLock(this.Id);
            }
            ...
            if (sci.TargetLocation != null)
            {
                ...
                this.operationState = undo ? StoreOperationState.UndoLocalTargetConnect : StoreOperationState.DoLocalTargetConnect;

                this.localConnectionTarget = this.Manager.StoreConnectionFactory.GetConnection(
                    StoreConnectionKind.LocalTarget,
                    this.GetConnectionStringForShardLocation(sci.TargetLocation));

                this.localConnectionTarget.OpenWithLock(this.Id);
            }
        }
