...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement
{
    ...
    internal class ReplaceMappingsGlobalOperation : StoreOperationGlobal
    {
        ...
        /// </summary>
        private IEnumerable<IStoreMapping> mappingsToRemove;
        ...
        private IEnumerable<IStoreMapping> GetMappingsToPurge(IStoreTransactionScope ts)
        {
            ...
            IStoreResults gsmMappingsByShard = ts.ExecuteOperation(
                ...
                StoreOperationRequestBuilder.GetAllShardMappingsGlobal(this.shardMap, this.shard, null));
            ...
            foreach (IStoreMapping lsmMapping in this.mappingsToRemove)
            {
                ShardKey min = ShardKey.FromRawValue(this.shardMap.KeyType, lsmMapping.MinValue);
                    ...
                    case ShardMapType.Range:
                        gsmMappingsByRange = ts.ExecuteOperation(
                            ...
                            StoreOperationRequestBuilder.GetAllShardMappingsGlobal(
                                ...
                                new ShardRange(
                                min,
                                ShardKey.FromRawValue(this.shardMap.KeyType, lsmMapping.MaxValue))));
                        break;
                ...
                else
                {
                    ...
                    {
                        ShardKey minGlobal = ShardKey.FromRawValue(this.shardMap.KeyType, gsmMappingByRange.MinValue);
                        ShardKey maxGlobal = null;

                        switch (this.shardMap.MapType)
                        {
                            case ShardMapType.Range:
                                maxGlobal = ShardKey.FromRawValue(this.shardMap.KeyType, gsmMappingByRange.MaxValue);
                                break;
                            default:
                                Debug.Assert(this.shardMap.MapType == ShardMapType.List);
                                maxGlobal = ShardKey.FromRawValue(this.shardMap.KeyType, gsmMappingByRange.MinValue).GetNextKey();
                                break;
                        }
