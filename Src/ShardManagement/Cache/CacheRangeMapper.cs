...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement
{
    ...
    internal class CacheRangeMapper : CacheMapper
    {
        ...
        /// </summary>
        private SortedList<ShardRange, CacheMapping> mappingsByRange;
        ...
        internal CacheRangeMapper(ShardKeyType keyType)
            : base(keyType)
        {
            this.mappingsByRange = new SortedList<ShardRange, CacheMapping>(Comparer<ShardRange>.Default);
        }
        ...
        internal override void AddOrUpdate(IStoreMapping sm, CacheStoreMappingUpdatePolicy policy)
        {
            ...
            this.mappingsByRange.Add(range, cm);
        }
        ...
        internal override void Remove(IStoreMapping sm)
        {
            ...
            if (this.mappingsByRange.ContainsKey(range))
            {
                this.mappingsByRange.Remove(range);
            }
            else
            {
                ...
                if (indexMax >= this.mappingsByRange.Keys.Count)
                {
                    indexMax = this.mappingsByRange.Keys.Count - 1;
                }

                // Find first range with max greater than min key.
                for (; indexMin <= indexMax; indexMin++)
                {
                    ShardRange currentRange = this.mappingsByRange.Keys[indexMin];
                ...
                {
                    ShardRange currentRange = this.mappingsByRange.Keys[indexMax];
                ...
                for (; indexMin <= indexMax; indexMin++)
                {
                    rangesToRemove.Add(this.mappingsByRange.Keys[indexMin]);
                }

                foreach (ShardRange rangeToRemove in rangesToRemove)
                {
                    this.mappingsByRange.Remove(rangeToRemove);
                }
            }
        }
        ...
        internal override ICacheStoreMapping LookupByKey(ShardKey key, out IStoreMapping sm)
        {
            ...
            if (rangeIndex != -1)
            {
                ShardRange range = this.mappingsByRange.Keys[rangeIndex];
                
                cm = this.mappingsByRange[range];
        ...
        {
            IList<ShardRange> rangeKeys = this.mappingsByRange.Keys;
