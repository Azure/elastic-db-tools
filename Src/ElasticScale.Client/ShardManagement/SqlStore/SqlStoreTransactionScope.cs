...
using Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement.Utils;
...
internal class SqlStoreTransactionScope : IStoreTransactionScope
{
    ...
    protected internal SqlStoreTransactionScope(StoreTransactionScopeKind kind, SqlConnection conn)
    {
        ...
        switch (Kind)
        {
            ...
            default:
                ...
                Debug.Assert(Kind == StoreTransactionScopeKind.NonTransactional);
                break;
        }
    }
    ...
    public virtual IStoreResults ExecuteOperation(string operationName, XElement operationData) => SqlUtils.WithSqlExceptionHandling<IStoreResults>(() =>
                                                                                                            {
                                                                                                                var results = new SqlResults();

                                                                                                                using (var cmd = _conn.CreateCommand())
                                                                                                                using (var input = operationData.CreateReader())
                                                                                                                {
                                                                                                                    cmd.Transaction = _tran;
                                                                                                                    cmd.CommandText = operationName;
                                                                                                                    cmd.CommandType = CommandType.StoredProcedure;

                                                                                                                    _ = SqlUtils.AddCommandParameter(cmd, "@input", SqlDbType.Xml, ParameterDirection.Input, -1, new SqlXml(input));

                                                                                                                    var result = SqlUtils.AddCommandParameter(cmd, "@result", SqlDbType.Int, ParameterDirection.Output, 0, 0);
    ...
    public virtual Task<IStoreResults> ExecuteOperationAsync(string operationName, XElement operationData) => SqlUtils.WithSqlExceptionHandlingAsync<IStoreResults>(async () =>
                                                                                                                       {
                                                                                                                           var results = new SqlResults();
    ...
    /// <returns>Storage results object.</returns>
    public virtual IStoreResults ExecuteCommandSingle(StringBuilder command) => SqlUtils.WithSqlExceptionHandling<IStoreResults>(() =>
                                                                                         {
                                                                                             var results = new SqlResults();
    ...
    IStoreResults IStoreTransactionScope.ExecuteOperation(string operationName, XElement operationData) => throw new NotImplementedException();
    Task<IStoreResults> IStoreTransactionScope.ExecuteOperationAsync(string operationName, XElement operationData) => throw new NotImplementedException();
    IStoreResults IStoreTransactionScope.ExecuteCommandSingle(StringBuilder command) => throw new NotImplementedException();
    IStoreResults IStoreTransactionScope.ExecuteOperation(string operationName, XElement operationData) => throw new NotImplementedException();
    Task<IStoreResults> IStoreTransactionScope.ExecuteOperationAsync(string operationName, XElement operationData) => throw new NotImplementedException();
    IStoreResults IStoreTransactionScope.ExecuteCommandSingle(StringBuilder command) => throw new NotImplementedException();
