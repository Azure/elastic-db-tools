...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.Query.Exceptions;
...
public class MultiShardAggregateException : Exception
{
    ...
    public MultiShardAggregateException(string message)
        : base(message) => _innerExceptions = new ReadOnlyCollection<Exception>(new Exception[0]);
    ...
    protected MultiShardAggregateException(SerializationInfo info, StreamingContext context)
        : base(info, context) => _innerExceptions = (ReadOnlyCollection<Exception>)info.GetValue("InnerExceptions", typeof(ReadOnlyCollection<Exception>));
    ...
    public override string ToString()
    {
        ...
        for (var i = 0; i < _innerExceptions.Count; i++)
            text = string.Format(
