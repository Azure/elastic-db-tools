...
public class ShardMapTests
{
    ...
    /// </summary>
    private static readonly string s_defaultShardMapName = "Customers_default";
    ...
    private static void CleanShardMapsHelper()
    {
        ...
        var sm = smm.GetShardMap(ShardMapTests.s_defaultShardMapName);
    ...
    public static void ShardMapTestsInitialize(TestContext testContext)
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            for (var i = 0; i < ShardMapTests.s_shardedDBs.Length; i++)
            {
                using (var cmd = new SqlCommand(
    ...
    /// </summary>
    [TestInitialize()]
    public void ShardMapTestInitialize() => ShardMapTests.CleanShardMapsHelper();

    /// <summary>
    /// Cleans up common state per-test.
    /// </summary>
    [TestCleanup()]
    public void ShardMapTestCleanup() => ShardMapTests.CleanShardMapsHelper();
    ...
    public void CreateShardDuplicate()
    {
        ...
        var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapTests.s_shardedDBs[0]);
        ...
        try
        {
            var sDuplicate = sm.CreateShard(sl);
        }
    ...
    public void DeleteShardVersionMismatch()
    {
        ...
        var su = new ShardUpdate
        {
            Status = ShardStatus.Offline
        };
        _ = sm.UpdateShard(sNew, su);
    ...
    public void UpdateShard()
    {
        ...
        var sNew = sm.CreateShard(new ShardCreationInfo(sl, ShardStatus.Online));
    ...
    public void UpdateShardVersionMismatch()
    {
        ...
        try
        {
            var sUpdatedFail = sm.UpdateShard(sNew, su);
        }
        catch (ShardManagementException sme)
        {
            Assert.AreEqual(ShardManagementErrorCategory.ShardMap, sme.ErrorCategory);
    ...
    public void ValidateShard()
    {
        ...
        var sUpdated = sm.UpdateShard(sNew, su);
        Assert.IsNotNull(sUpdated);

        var validationFailed = false;
        try
        {
            using var conn = sNew.OpenConnection(
                Globals.ShardUserConnectionString,
                ConnectionOptions.Validate);
        }
        catch (ShardManagementException sme)
        ...
        try
        {
            using var conn = sUpdated.OpenConnection(
                Globals.ShardUserConnectionString,
                ConnectionOptions.Validate);
        }
    ...
    public void CreateShardAbortGSM()
    {
        ...
        void eventHandler(object sender, RetryingEventArgs arg) => retryCount++;
        ...
        var count = 0;
        var sList = sm.GetShards();

        using (var sEnum = sList.GetEnumerator())
        {
            while (sEnum.MoveNext())
                count++;
        }
    ...
    public void CreateShardAbortGSMDoAndLSMUndo()
    {
        ...
        IStoreOperationFactory sof = new StubStoreOperationFactory()
        {
            ...
            (_smm, _sm, _s) =>
            {
                var op = new StubAddShardOperation(_smm, _sm, _s)
                {
                    CallBase = true
                };
    ...
    public void DeleteShardAbortGSMDoAndLSMUndo()
    {
        ...
        IStoreOperationFactory sof = new StubStoreOperationFactory()
        {
            ...
            (_smm, _sm, _s) =>
            {
                var op = new StubRemoveShardOperation(_smm, _sm, _s)
                {
                    CallBase = true
                };
                op.DoGlobalPostLocalExecuteIStoreTransactionScope = (ts) =>
    ...
    public void UpdateShardAbortGSMDoAndLSMUndo()
    {
        ...
        IStoreOperationFactory sof = new StubStoreOperationFactory()
        {
            ...
            (_smm, _sm, _so, _sn) =>
            {
                var op = new StubUpdateShardOperation(_smm, _sm, _so, _sn)
                {
                    CallBase = true
                };
                op.DoGlobalPostLocalExecuteIStoreTransactionScope = (ts) =>
