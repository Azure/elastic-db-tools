...
internal class ShardKeyInfo
{
    ...
    /// </summary>
    public ShardKey ShardKeyFromValue => new(KeyType, Value);

    /// <summary>
    /// Gets the ShardKey using ShardKey.FromRawValue(keyType, rawValue);
    /// </summary>
    public ShardKey ShardKeyFromRawValue => ShardKey.FromRawValue(KeyType, RawValue);

    public override string ToString() => string.Format("[{0}] {1} <-> {2}", KeyType, Value, ToString(RawValue));

    private static string ToString(byte[] bytes) => bytes == null ? "(null)" : "0x" + string.Join(string.Empty, bytes.Select(b => b.ToString("x2")));
