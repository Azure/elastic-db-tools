...
public class ShardMapManagerUpgradeTests
{
    ...
    /// </summary>
    private static readonly string[] s_shardMapNames = new[]
    {
        "shardMap1", "shardMap2"
    };
    ...
    /// </summary>
    private static readonly Version s_initialGsmVersion = new(1, 0);

    /// <summary>
    /// initial LSM version to deploy.
    /// </summary>
    private static readonly Version s_initialLsmVersion = new(1, 0);
    ...
    public void ShardMapManagerUpgradeTestInitialize()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            for (var i = 0; i < ShardMapManagerUpgradeTests.s_shardedDBs.Length; i++)
            {
                using (var cmd = new SqlCommand(
    ...
    public void GetDistinctLocations()
    {
        ...
        // create shard maps
        foreach (var name in ShardMapManagerUpgradeTests.s_shardMapNames)
        {
            ShardMap sm = smm.CreateListShardMap<int>(name);
            Assert.IsNotNull(sm);
        }

        var sm1 = smm.GetShardMap(ShardMapManagerUpgradeTests.s_shardMapNames[0]);
        Assert.IsNotNull(sm1);

        var sm2 = smm.GetShardMap(ShardMapManagerUpgradeTests.s_shardMapNames[1]);
        Assert.IsNotNull(sm2);

        // Add shards to the shard maps.

        var sl1 = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapManagerUpgradeTests.s_shardedDBs[0]);
        var sl2 = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapManagerUpgradeTests.s_shardedDBs[1]);
        var sl3 = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapManagerUpgradeTests.s_shardedDBs[2]);
        _ = sm1.CreateShard(sl1);
        _ = sm1.CreateShard(sl2);
        _ = sm1.CreateShard(sl3);
        _ = sm2.CreateShard(sl2);
        _ = sm2.CreateShard(sl3);
    ...
    public void UpgradeLsm()
    {
        ...
        var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, s_shardedDBs[0]);
        ...
        var listsm = smm.CreateListShardMap<int>(ShardMapManagerUpgradeTests.s_shardMapNames[0]);

        _ = listsm.CreateShard(sl);
    ...
    public void TestLockingFixInVersion1_2()
    {
        ...
        var rsm = smm.CreateRangeShardMap<int>(ShardMapManagerUpgradeTests.s_shardMapNames[1]);
        Assert.IsNotNull(rsm);

        var s = rsm.CreateShard(new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapManagerUpgradeTests.s_shardedDBs[0]));
        Assert.IsNotNull(s);

        var m1 = rsm.CreateRangeMapping(new Range<int>(1, 10), s);
        var m2 = rsm.CreateRangeMapping(new Range<int>(10, 20), s);
        var m3 = rsm.CreateRangeMapping(new Range<int>(20, 30), s);

        // Lock first 2 mappings with same lockownerid and third with a different lock owner id

        var t1 = MappingLockToken.Create();
        var t2 = MappingLockToken.Create();
        ...
        foreach (var m in rsm.GetMappings())
        {
    ...
    private void VerifyGlobalStore(ShardMapManager smm, Version targetVersion)
    {
        ...
        var shardMapName = string.Format("MyShardMap_{0}", Guid.NewGuid());
        if (targetVersion != null && targetVersion < new Version(1, 1))
        {
            var sme = AssertExtensions.AssertThrows<ShardManagementException>(
    ...
    private Version GetGlobalStoreVersion() => GetVersion(SqlUtils.CheckIfExistsGlobalScript.Single().ToString());

    private Version GetVersion(string getVersionScript)
    {
        using var conn = new SqlConnection(Globals.ShardMapManagerConnectionString);
        conn.Open();
        var cmd = new SqlCommand(getVersionScript, conn);
        using var reader = cmd.ExecuteReader();
        Assert.IsTrue(reader.Read());
        return reader.FieldCount == 2
            ? new Version(reader.GetInt32(1), 0)
            : reader.FieldCount == 3
                ? new Version(reader.GetInt32(1), reader.GetInt32(2))
