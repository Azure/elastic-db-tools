...
public class RecoveryManagerTests
{
    ...
    /// </summary>
    private static readonly string[] s_gsmTables = new[]
    {
    ...
    /// </summary>
    private static readonly string[] s_lsmTables = new[]
    {
    ...
    private static void CreateShardMapsHelper()
    {
        ...
        var lsm = smm.CreateListShardMap<int>(RecoveryManagerTests.s_listShardMapName);
        ...
        var rsm = smm.CreateRangeShardMap<int>(RecoveryManagerTests.s_rangeShardMapName);
    ...
    private static void CleanTablesHelper()
    {
        ...
        // Clean LSM tables.
        foreach (var dbName in RecoveryManagerTests.s_shardedDBs)
        {
            foreach (var tableName in s_lsmTables)
            {
        ...
        // Clean GSM tables
        foreach (var tableName in s_gsmTables)
        {
            using var cmd = new SqlCommand(
    ...
    public static void RecoveryManagerTestsInitialize(TestContext testContext)
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            for (var i = 0; i < RecoveryManagerTests.s_shardedDBs.Length; i++)
            {
                using (var cmd = new SqlCommand(
    ...
    /// </summary>
    [TestInitialize()]
    public void ShardMapperTestInitialize() => CreateShardMapsHelper();

    /// <summary>
    /// Cleans up common state per-test.
    /// </summary>
    [TestCleanup()]
    public void ShardMapperTestCleanup() => CleanTablesHelper();
    ...
    public void TestDetachAttachShard()
    {
        ...
        var rsm = smm.GetRangeShardMap<int>(RecoveryManagerTests.s_rangeShardMapName);

        Assert.IsNotNull(rsm);

        var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[0]);
    ...
    public void TestConsistencyDetectionAndViewingWithVersionOnlyConflict()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("update {0}.__ShardManagement.ShardMappingsGlobal set MappingId = newid()", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestConsistencyDetectionAndViewingWithWiderRangeInLSM()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand("update shard1.__ShardManagement.ShardMappingsLocal set MinValue = MinValue - 1, MaxValue = MaxValue + 1", conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestConsistencyDetectionAndViewingWithWiderRangeInGSM()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("update {0}.__ShardManagement.ShardMappingsGlobal set MinValue = MinValue - 1, MaxValue = MaxValue + 1", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestConsistencyDetectionAndViewingWithAdditionalRangeInLSM()
    {
        ...
        _ = rsm.CreateRangeMapping(new Range<int>(11, 20), s);
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("delete from {0}.__ShardManagement.ShardMappingsGlobal where MinValue = 0x8000000B", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestConsistencyDetectionOnListMapping()
    {
        ...
        var rsm = smm.GetListShardMap<int>(RecoveryManagerTests.s_listShardMapName);
        ...
        for (var i = 0; i < 5; i++)
        {
            var p = rsm.CreatePointMapping(2 * i, s);
            Assert.IsNotNull(p);
        }
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using (var cmd = new SqlCommand(string.Format("delete from {0}.__ShardManagement.ShardMappingsGlobal where MinValue IN (0x80000000, 0x80000002)", Globals.ShardMapManagerDatabaseName), conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("delete from shard1.__ShardManagement.ShardMappingsLocal where MinValue = 0x80000008", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("update shard1.__ShardManagement.ShardMappingsLocal set MappingId = newid() where MinValue = 0x80000006", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }
        }
    ...
    public void TestConsistencyDetectionAndViewingWithDisjointRanges()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("delete from {0}.__ShardManagement.ShardMappingsGlobal where MinValue = 0x80000001", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("delete from shard1.__ShardManagement.ShardMappingsLocal where MinValue = 0x8000000B", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestConsistencyDetectionWithDivergence()
    {
        ...
        var sl1 = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[0]);
        var sl2 = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[1]);
        ...
        _ = rsm.CreateRangeMapping(new Range<int>(1, 6), s1);
        _ = rsm.CreateRangeMapping(new Range<int>(6, 10), s2);
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand("update shard1.__ShardManagement.ShardMappingsLocal set MaxValue = 0x8000000B", conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestCopyGSMToLSM()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand("delete from shard1.__ShardManagement.ShardMappingsLocal", conn);
            _ = cmd.ExecuteNonQuery();
        }
        ...
        var gsAfterFix = rm.DetectMappingDifferences(sl);
        foreach (var g in gsAfterFix)
        {
            var kvps = rm.GetMappingDifferences(g);
    ...
    public void TestCopyLSMToGSM()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("delete from {0}.__ShardManagement.ShardMappingsGlobal", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestRestoreGSMFromLSMsRange()
    {
        ...
        var i = 0;
        var ranges = new List<RangeMapping<int>>();
        foreach (var dbName in RecoveryManagerTests.s_shardedDBs)
        {
            var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, dbName);
            sls.Add(sl);
            var s = rsm.CreateShard(sl);
            Assert.IsNotNull(s);
    ...
    public void TestRestoreGSMFromLSMsRangeWithGarbageInGSM()
    {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using var cmd = new SqlCommand(string.Format("update {0}.__ShardManagement.ShardMappingsGlobal set MaxValue = MaxValue + 1, MinValue = MinValue + 1", Globals.ShardMapManagerDatabaseName), conn);
            _ = cmd.ExecuteNonQuery();
        }
    ...
    public void TestRestoreGSMFromLSMsList()
    {
        ...
        IList<ShardLocation> sls = new List<ShardLocation>();
        var i = int.MaxValue;
        var points = new List<PointMapping<int>>();
        foreach (var dbName in RecoveryManagerTests.s_shardedDBs)
        {
            var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, dbName);
            sls.Add(sl);
    ...
    public void TestRebuildShardFromGSMRange()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            var r = rsm.CreateRangeMapping(new Range<int>(1 + i, 2 + i), s);
            Assert.IsNotNull(r);
        }
    ...
    public void TestRebuildShardFromGSMRangeKeepNonconflicts()
    {
        ...
        // set initial ranges as non-intersecting.
        _ = rsm.CreateRangeMapping(new Range<int>(1, 6), s1);
        _ = rsm.CreateRangeMapping(new Range<int>(6, 10), s1);
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using (var cmd = new SqlCommand("update shard1.__ShardManagement.ShardMappingsLocal set MaxValue = 0x8000000B, MappingId = newid() where MaxValue = 0x8000000A", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("update shard1.__ShardManagement.ShardMappingsLocal set MinValue = 0x8000000B where MinValue = 0x8000000A", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }
        }
    ...
    public void TestRebuildShardFromGSMList()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            var r = lsm.CreatePointMapping(i, s);
            Assert.IsNotNull(r);
        }
    ...
    public void TestPointMappingRecoverFromLSM()
    {
        ...
        var listsm = smm.GetListShardMap<int>(RecoveryManagerTests.s_listShardMapName);

        Assert.IsNotNull(listsm);

        var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[0]);

        var s = listsm.CreateShard(sl);

        Assert.IsNotNull(s);

        for (var i = 0; i < 5; i++)
        {
            var r = listsm.CreatePointMapping(creationInfo: new PointMappingCreationInfo<int>(i, s, MappingStatus.Online));
            Assert.IsNotNull(r);
        }
    ...
    public void TestGeoFailoverAttach()
    {
        ...
        var sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[0]);
        var slNew = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, RecoveryManagerTests.s_shardedDBs[0] + "_new");
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using (var cmd = new SqlCommand("alter database shard1 set single_user with rollback immediate", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("alter database shard1 modify name = shard1_new", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("alter database shard1_new set multi_user with rollback immediate", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }
        }
        ...
        using (var op = smm.StoreOperationFactory.CreateGetShardsLocalOperation(
            smm,
            slNew,
            "RecoveryTest"))
        {
        ...
        using (var conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
        {
            ...
            using (var cmd = new SqlCommand("alter database shard1_new set single_user with rollback immediate", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("alter database shard1_new modify name = shard1", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }

            using (var cmd = new SqlCommand("alter database shard1 set multi_user with rollback immediate", conn))
            {
                _ = cmd.ExecuteNonQuery();
            }
        }
