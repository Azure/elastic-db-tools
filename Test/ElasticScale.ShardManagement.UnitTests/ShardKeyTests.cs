...
public class ShardKeyTests
{
    ...
    /// </summary>
    private readonly Dictionary<ShardKeyType, int> _shardKeyTypeLength = new()
    {
    ...
    {
        foreach (var shardKeyInfo in ShardKeyInfo.AllTestShardKeyInfos)
        {
    ...
    public void TestShardKeySerialization()
    {
        foreach (var shardKeyInfo in ShardKeyInfo.AllTestShardKeyInfos)
        {
            ...
            var expectedSerializedValue = shardKeyInfo.RawValue;
            var actualSerializedValue = shardKeyInfo.ShardKeyFromValue.RawValue;
    ...
    public void TestShardKeyDeserialization()
    {
        foreach (var shardKeyInfo in ShardKeyInfo.AllTestShardKeyInfos)
        {
            ...
            var expectedDeserializedShardKey = shardKeyInfo.ShardKeyFromValue;
            var actualDeserializedShardKey = shardKeyInfo.ShardKeyFromRawValue;
    ...
    public void TestShardKeyDeserializationAddTrailingZeroes()
    {
        foreach (var shardKeyInfo in ShardKeyInfo.AllTestShardKeyInfos)
        {
            ...
            var dataTypeLength = _shardKeyTypeLength[shardKeyInfo.KeyType];
            if (shardKeyInfo.RawValue != null && shardKeyInfo.RawValue.Length != dataTypeLength)
            {
                // Add trailing zeroes
                var originalRawValue = shardKeyInfo.RawValue;
                var rawValueWithTrailingZeroes = new byte[dataTypeLength];
                originalRawValue.CopyTo(rawValueWithTrailingZeroes, 0);

                var expectedDeserializedShardKey = shardKeyInfo.ShardKeyFromValue;
                var actualDeserializedShardKey = ShardKey.FromRawValue(shardKeyInfo.KeyType, rawValueWithTrailingZeroes);
    ...
    public void TestShardKeyTypeInfo()
    {
        foreach (var shardKeyTypeInfo in ShardKeyTypeInfo.ShardKeyTypeInfos.Values)
        {
            ...
            if (shardKeyTypeInfo.KeyType is not ShardKeyType.DateTime and not ShardKeyType.DateTimeOffset)
            {
    ...
    {
        for (var i = 0; i < _orderedGuidsDescending.Length - 1; i++)
        {
            Assert.IsTrue(
    ...
    {
        var dateTimeOffsetShardKeyInfos =
            ShardKeyInfo.AllTestShardKeyInfosGroupedByType[ShardKeyType.DateTimeOffset]
            .ToArray();

        for (var i = 0; i < dateTimeOffsetShardKeyInfos.Length - 1; i++)
        {
            var low = dateTimeOffsetShardKeyInfos[i];
            var high = dateTimeOffsetShardKeyInfos[i + 1];
    ...
    {
        var testValue = TimeSpan.FromMinutes(6.2);
    ...
    {
        var testValue = DateTimeOffset.Now;
        TestShardKeyGeneric<DateTimeOffset>(ShardKeyType.DateTimeOffset, testValue, typeof(DateTimeOffset));

        var d1 = DateTime.Now;
        var k1 = new ShardKey(new DateTimeOffset(d1, DateTimeOffset.Now.Offset));
        var k2 = new ShardKey(new DateTimeOffset(d1.ToUniversalTime(), TimeSpan.FromHours(0)));
        Assert.AreEqual(k1, k2);

        var k3 = ShardKey.MinDateTimeOffset;
    ...
    private void TestShardKeyGeneric<TKey>(ShardKeyType keyType, TKey inputValue, Type realType)
    {
        ...
        //
        var k1 = new ShardKey(inputValue);
        Assert.AreEqual(realType, k1.DataType);

        // Go to/from raw value
        var k2 = new ShardKey(keyType, inputValue);
        var k2raw = k2.RawValue;

        var k3 = ShardKey.FromRawValue(keyType, k2raw);
