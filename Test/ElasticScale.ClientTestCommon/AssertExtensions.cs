...
where TExceptionOuter : Exception where TExceptionInner : Exception
    {
        ...
        {
            if (ex is not TExceptionOuter exceptionOuter)
                throw;
            ...
            do
            {
                if (ex is TExceptionInner exceptionInner)
                {
    ...
    /// </summary>
    public static TException WaitAndAssertThrows<TException>(Task task) where TException : Exception => WaitAndAssertThrows<TException>(task, TimeSpan.FromMilliseconds(-1));
    ...
    public static TException WaitAndAssertThrows<TException>(Task task, TimeSpan timeout) where TException : Exception
    {
        ...
        catch (AggregateException ex)
        {
            ...
            {
                if (ex.InnerException is TException innerException)
    ...
    public static string ToCommaSeparatedString<T>(this IEnumerable<T> collection) => string.Join(", ", collection);

    /// <summary>
    /// Asserts that the collection contains the expected item.
    /// </summary>
    public static void AssertContains<T>(T item, IEnumerable<T> collection) => AssertContains(item, collection, EqualityComparer<T>.Default);
    ...
    public static void AssertContains(string substring, string value)
    {
        ...
        var substringIndex = value.IndexOf(substring, StringComparison.Ordinal);
        if (substringIndex >= 0)
            // Success
            return;
    ...
    /// Asserts that the collection does not contain the expected item.
    /// </summary>
    public static void AssertDoesNotContain<T>(T item, IEnumerable<T> collection) => AssertDoesNotContain(item, collection, EqualityComparer<T>.Default);
    ...
    /// </summary>
    public static void AssertSequenceEqual<T>(IEnumerable<T> expected, IEnumerable<T> actual, string message = "") => AssertSequenceEqual(expected, actual, EqualityComparer<T>.Default, message);
    ...
    /// are the same.
    /// </summary>
    public static void AssertSequenceEquivalent<T>(IEnumerable<T> expected, IEnumerable<T> actual, string message = "") => AssertSequenceEquivalent(expected, actual, EqualityComparer<T>.Default, message);
    ...
    public static void AssertSequenceEquivalent<T>(IEnumerable<T> expected, IEnumerable<T> actual, IEqualityComparer<T> equalityComparer, string message = "")
    {
        ...
        {
            var expectedSet = new HashSet<T>(expectedArray, equalityComparer);
            var expectedSetContainsAllActual = actualArray.All(a => expectedSet.Contains(a, equalityComparer));
