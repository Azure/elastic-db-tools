...
//
// @File: MultiShardQueryE2ETests.cs
//
// @Owner: raveeram
// @Test:
//
...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.Query.UnitTests
{
    ...
    public class MultiShardQueryE2ETests
    {
        ...
        /// </summary>
        private MultiShardConnection m_shardConnection;

        /// <summary>
        /// Handle to the ShardMap with our Test databases.
        /// </summary>
        private ShardMap m_shardMap;
        ...
        {
            m_shardMap = MultiShardTestUtils.CreateAndGetTestShardMap();

            m_shardConnection = new MultiShardConnection(m_shardMap.GetShards(), MultiShardTestUtils.ShardConnectionString);
        }
        ...
        public void TestSimpleSelect(MultiShardExecutionPolicy policy)
        {
            ...
            //
            using (MultiShardConnection conn = new MultiShardConnection(m_shardMap.GetShards(), MultiShardTestUtils.ShardConnectionString))
            {
        ...
        public void TestSelect_Failure_PartialResults()
        {
            ...
            Assert.AreEqual(m_shardMap.GetShards().Count(), e.InnerExceptions.Count);
        }
        ...
        public void TestSelect_Failure_CompleteResults()
        {
            ...
            AssertExtensions.AssertLessThanOrEqualTo(m_shardMap.GetShards().Count(), e.InnerExceptions.Count);
        }
        ...
        {
            IEnumerable<ShardLocation> shardLocations = m_shardMap.GetShards().Select(s => s.Location);
        ...
        /// <summary>
        /// Basic test for ensuring that we don't blow up due to a schema mismatch on the shards.
        /// </summary>
        [TestMethod]
        [TestCategory("ExcludeFromGatedCheckin")]
        ...
        public void TestQueryShardsInvalidConnectionSync()
        {
            ...
            try
            {
                using (var conn = new MultiShardConnection(m_shardMap.GetShards(), MultiShardTestUtils.ShardConnectionString))
                {
...
END";
                using (var cmd = m_shardConnection.CreateCommand())
                {
...
drop type dbo.PageViewTableType
        ...
        public void TestQueryShardsCommandCancellationHandler()
        {
            ...
            using (MultiShardCommand cmd = m_shardConnection.CreateCommand())
            {
                ...
                List<ShardLocation> allShards = m_shardConnection.ShardConnections.Select(l => l.Item1).ToList();
        ...
        public void TestQueryShardsInvalidShardStateSync()
        {
            ...
            var shardSqlConnections = m_shardConnection.ShardConnections;
        ...
        public void TestInvalidMultiShardConnectionString()
        {
            ...
            try
            {
                conn = new MultiShardConnection(m_shardMap.GetShards(), connectionString: null);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                conn = new MultiShardConnection(m_shardMap.GetShards(), MultiShardTestUtils.ShardMapManagerConnectionString);
            }
            catch (Exception ex)
            {
            ...
            conn = new MultiShardConnection(m_shardMap.GetShards(), connStringBldr.ConnectionString);
        ...
        private List<Tuple<ShardLocation, DbConnection>> GetProxyShardConnections(ProxyServer proxy)
        {
            ...
            foreach (Shard shard in m_shardMap.GetShards())
            {
