...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.Query.UnitTests
{
    ...
    internal static class MultiShardTestUtils
    {
        ...
        /// </summary>
        private static string testTableName = "ConsistentShardedTable";
        ...
        /// </summary>
        private static SqlCredential testCredential = GenerateDefaultSqlCredential();
        ...
        /// </summary>
        private static string TestShardMapName = "TestShardMap";
        ...
        internal static void CreateAndPopulateTables()
        {
            ...
            {
                string dbName = TestDatabaseNames[i];
        ...
        internal static void DropAndCreateDatabases()
        {
            ...
            //
            AddDropAndCreateDatabaseCommandsToList(ShardMapManagerDbName, commands);
        ...
        internal static void DropDatabases()
        {
            ...
            //
            commands.Add(DropDatabaseCommand(ShardMapManagerDbName));
        ...
        {
            using (SqlConnection conn = new SqlConnection(GetTestConnectionString(TestDatabaseNames[database])))
            {
                conn.Open();

                string tsql = String.Format("EXEC sp_rename '[{0}].[{1}]', '{2}', 'COLUMN';",
                    testTableName, oldColName, newColName);
        ...
        internal static ShardMap CreateAndGetTestShardMap()
        {
            ...
            sm = smm.CreateListShardMap<int>(TestShardMapName);
            for (int i = 0; i < TestDatabaseNames.Count; i++)
            {
                sm.CreateShard(GetTestShard(TestDatabaseNames[i]));
            }
            return sm;
        }
        ...
        private static string GetTestConnectionString(string database)
        {
            ...
            builder.DataSource = serverLocation;
        ...
        {
            Assert.IsNotNull(database, "null database");
            ShardLocation rVal = new ShardLocation(serverLocation, database);
            return rVal;
        }
        ...
        private static SqlCredential GenerateDefaultSqlCredential()
        {
            ...
            char[] pwdChars = testPassword.ToCharArray();
            ...
            return new SqlCredential(testUserId, ss);
        }
        ...
        private static string DropLoginCommand()
        {
            return string.Format(
                "IF EXISTS (SELECT name FROM sys.sql_logins WHERE name = N'{0}') DROP LOGIN {0}", testUserId);
        }
        ...
        private static void AddDropAndReCreateTestUserCommandsToList(List<string> output)
        {
            ...
            // Then re create it.
            //
            output.Add(string.Format("CREATE LOGIN {0} WITH Password = '{1}';", testUserId, testPassword));

            // Then grant it lots of permissions.
            //
            output.Add(string.Format("GRANT CONTROL SERVER TO {0}", testUserId));
        }
        ...
        private static string GetTestTableCreateCommand()
        {
            ...
            //
            createCommand.AppendFormat("CREATE TABLE {0} ({1} nvarchar(50)", testTableName, dbNameField);
        ...
        private static string[] GetInsertValuesCommands(int numCommands, string dbName)
        {
            ...
            for (int i = 0; i < numCommands; i++)
            {
                ...
                //
                insertCommand.AppendFormat("INSERT INTO {0} ({1}", testTableName, dbNameField);
        ...
        {
            int theValue = random.Next(Int16.MinValue, Int16.MaxValue);
            return GetSpecificIntCastAsArg(theValue, column);
        }
        ...
        {
            string theVal = (random.Next() > random.Next()) ? "TRUE" : "FALSE";
        ...
        {
            double randomDouble = random.NextDouble() * double.MaxValue;
        ...
        private static string GetRandomSqlDecimalValue(MutliShardTestCaseColumn column)
        {
            ...
            //
            double theValue = random.NextDouble() * Decimal.ToDouble(Decimal.MaxValue);
        ...
        {
            return random.Next(Byte.MinValue, Byte.MaxValue).ToString();
        }
