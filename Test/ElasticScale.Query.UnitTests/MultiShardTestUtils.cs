...
internal static class MultiShardTestUtils
{
    ...
    /// </summary>
    private static readonly string s_serverLocation = "localhost";

    /// <summary>
    /// UserId to use when connecting to shards during a fanout query.
    /// </summary>
    private static readonly string s_testUserId = "TestUser";

    /// <summary>
    /// User password to use when connecting to shards during a fanout query.
    /// </summary>
    private static readonly string s_testPassword = "dogmat1C";

    /// <summary>
    /// Table name for the sharded table we will issue fanout queries against.
    /// </summary>
    private static readonly string s_testTableName = "ConsistentShardedTable";

    /// <summary>
    /// Field on the sharded table where we will store the database name.
    /// </summary>
    private static readonly string s_dbNameField = "dbNameField";
    ...
    /// </summary>
    private static readonly string s_shardMapManagerDbName = "ShardMapManager";

    /// <summary>
    /// Name of the test shard map to use.
    /// </summary>
    private static readonly string s_testShardMapName = "TestShardMap";

    /// <summary>
    /// List containing the names of the test databases.
    /// </summary>
    private static readonly List<string> s_testDatabaseNames = GenerateTestDatabaseNames();
    ...
    internal static void CreateAndPopulateTables()
    {
        ...
        for (var i = 0; i < s_testDatabaseNames.Count; i++)
        {
            var dbName = s_testDatabaseNames[i];
            commands.Add(string.Format("USE {0};", dbName));

            // First create the table.
            //
            var createTable = GetTestTableCreateCommand();
            commands.Add(createTable);

            // Then add the records.
            //
            var insertValuesCommands = GetInsertValuesCommands(3, dbName);
            foreach (var insertValuesCommand in insertValuesCommands)
            {
    ...
    {
        using var conn = new SqlConnection(GetTestConnectionString(s_testDatabaseNames[database]));
        conn.Open();

        var tsql = string.Format("EXEC sp_rename '[{0}].[{1}]', '{2}', 'COLUMN';",
    ...
    {
        ShardMap sm;
        _ = ShardMapManagerFactory.CreateSqlShardMapManager(MultiShardTestUtils.ShardMapManagerConnectionString, ShardMapManagerCreateMode.ReplaceExisting);
        var smm = ShardMapManagerFactory.GetSqlShardMapManager(MultiShardTestUtils.ShardMapManagerConnectionString, ShardMapManagerLoadPolicy.Lazy);

        sm = smm.CreateListShardMap<int>(s_testShardMapName);
        for (var i = 0; i < s_testDatabaseNames.Count; i++)
        {
            _ = sm.CreateShard(GetTestShard(s_testDatabaseNames[i]));
        }
    ...
    {
        Assert.IsNotNull(database, "null database");
        var rVal = new ShardLocation(s_serverLocation, database);
        return rVal;
    }
    ...
    private static string DropDatabaseCommand(string dbName) => string.Format("IF EXISTS (SELECT name FROM sys.databases WHERE name = N'{0}') DROP DATABASE [{0}]", dbName);
    ...
    private static string CreateDatabaseCommand(string dbName) => string.Format("CREATE DATABASE [{0}]", dbName);
    ...
    private static string DropLoginCommand() => string.Format(
    ...
    {
        using var toExecute = theConn.CreateCommand();
        toExecute.CommandText = theCommand;
        _ = toExecute.ExecuteNonQuery();
    }
    ...
    {
        using var theConn = new SqlConnection(GetTestConnectionString(initialCatalog));
        theConn.Open();

        using (var toExecute = theConn.CreateCommand())
        {
            foreach (var tsql in theCommands)
            {
    ...
    private static string GetTestTableCreateCommand()
    {
        ...
        //
        _ = createCommand.AppendFormat("CREATE TABLE {0} ({1} nvarchar(50)", s_testTableName, s_dbNameField);

        var fieldInfo = MutliShardTestCaseColumn.DefinedColumns;
        for (var i = 0; i < fieldInfo.Count; i++)
        {
            var curField = fieldInfo[i];
            _ = createCommand.AppendFormat(", {0} {1}", curField.TestColumnName, curField.ColumnTypeDeclaration);
        }

        _ = createCommand.Append(");");
    ...
    {
        var commandsToReturn = new string[numCommands];
        ...
        {
            _ = insertCommand.Clear();

            // Set up the stem, which includes putting the dbName in the dbNameColumn.
            //
            _ = insertCommand.AppendFormat("INSERT INTO {0} ({1}", s_testTableName, s_dbNameField);

            // Now put in all the column names in the order we will do them.
            //
            for (var j = 0; j < fieldInfo.Count; j++)
            {
                var curField = fieldInfo[j];
                ...
                {
                    _ = insertCommand.AppendFormat(", {0}", curField.TestColumnName);
                }
            }

            // Close the field list
            //
            _ = insertCommand.Append(") ");

            // Now put in the VALUES stem
            //
            _ = insertCommand.AppendFormat("VALUES ('{0}'", dbName);
            ...
            for (var j = 0; j < fieldInfo.Count; j++)
            {
                ...
                if (!IsTimestampField(curField.DbType))
                {
                    var valueToPutIn = GetTestFieldValue(curField);
                    _ = insertCommand.AppendFormat(", {0}", valueToPutIn);
                }
            }
    ...
    {
        var dbType = dataTypeInfo.DbType;
        int? length = dataTypeInfo.FieldLength;

        return dbType switch
        {
            SqlDbType.BigInt => GetRandomIntCastAsArg(dataTypeInfo),// SQL Types: bigint
            SqlDbType.Binary => GetRandomSqlBinaryValue(length.Value),// SQL Types: image
            SqlDbType.Bit => GetRandomSqlBitValue(),// SQL Types: bit
            SqlDbType.Char => GetRandomSqlCharValue(length.Value),// SQL Types: char[(n)]
            SqlDbType.Date => GetRandomSqlDateValue(),// SQL Types: date
            SqlDbType.DateTime => GetRandomSqlDatetimeCastAsArg(dataTypeInfo),// SQL Types: datetime, smalldatetime
            SqlDbType.DateTime2 => GetRandomSqlDatetimeCastAsArg(dataTypeInfo),// SQL Types: datetime2
            SqlDbType.DateTimeOffset => GetRandomSqlDatetimeoffsetValue(),// SQL Types: datetimeoffset
            SqlDbType.Decimal => GetRandomSqlDecimalValue(dataTypeInfo),// SQL Types: decimal, numeric
                                                                        // These are the same.
            SqlDbType.Float => GetRandomSqlFloatValue(dataTypeInfo),// SQL Types: float
            SqlDbType.Image => GetRandomSqlBinaryValue(dataTypeInfo.FieldLength),// SQL Types: image
            SqlDbType.Int => GetRandomSqlIntValue(),// SQL Types: int
            SqlDbType.Money => GetRandomSqlMoneyValue(dataTypeInfo),// SQL Types: money
            SqlDbType.NChar => GetRandomSqlNCharValue(length.Value),// SQL Types: nchar[(n)]
            SqlDbType.NText => GetRandomSqlNCharValue(length.Value),// SQL Types: ntext
            SqlDbType.NVarChar => GetRandomSqlNCharValue(length.Value),// SQL Types: nvarchar[(n)]
            SqlDbType.Real => GetRandomSqlRealValue(dataTypeInfo),// SQL Types: real
            SqlDbType.SmallDateTime => GetRandomSqlDatetimeCastAsArg(dataTypeInfo),// SQL Types: smalldatetime
            SqlDbType.SmallInt => GetRandomSqlSmallIntValue(dataTypeInfo),// SQL Types: smallint
            SqlDbType.SmallMoney => GetRandomSqlSmallMoneyValue(dataTypeInfo),// SQL Types: smallmoney
            SqlDbType.Text => GetRandomSqlCharValue(length.Value),// SQL Types: text
            SqlDbType.Time => GetRandomSqlDatetimeCastAsArg(dataTypeInfo),// SQL Types: time
            SqlDbType.Timestamp => throw new ArgumentException(SqlDbType.Timestamp.ToString()),// SQL Types: rowversion, timestamp
                                                                                               //exclding it should happen automatically.  should not be here. throw.
            SqlDbType.TinyInt => GetRandomSqlTinyIntValue(),// SQL Types: tinyint
            SqlDbType.UniqueIdentifier => GetRandomSqlUniqueIdentifierValue(),// SQL Types: uniqueidentifier
            SqlDbType.VarBinary => GetRandomSqlBinaryValue(length.Value),// SQL Types: binary[(n)], varbinary[(n)]
            SqlDbType.VarChar => GetRandomSqlCharValue(length.Value),// SQL Types: varchar[(n)]
            _ => throw new ArgumentException(dbType.ToString()),
        };
    }
    ...
    {
        var theValue = s_random.Next(short.MinValue, short.MaxValue);
        return GetSpecificIntCastAsArg(theValue, column);
    }
    ...
    /// <returns>The tsql fragment to generate the desired value.</returns>
    private static string GetSpecificIntCastAsArg(int theValue, MutliShardTestCaseColumn column) => string.Format("CAST({0} AS {1})", theValue, column.SqlServerDatabaseEngineType);
    ...
    private static string GetRandomSqlBinaryValue(int length)
    {
        ...
        var bytesAsString = BitConverter.ToString(rawData);
    ...
    {
        var theVal = (s_random.Next() > s_random.Next()) ? "TRUE" : "FALSE";
    ...
    /// <returns>The tsql fragment to generate the desired value.</returns>
    private static string GetRandomSqlCharValue(int length) => string.Format("'{0}'", GetRandomString(length));

    /// <summary>
    /// Helper that produces a random SqlDateValue.
    /// </summary>
    /// <returns>The tsql to produce the value.</returns>
    private static string GetRandomSqlDateValue() => "GETDATE()";

    /// <summary>
    /// Helper that produces a random SqlDatetime value.
    /// </summary>
    /// <returns>The tsql to produce the value.</returns>
    private static string GetRandomSqlDatetimeValue() => "SYSDATETIME()";
    ...
    private static string GetRandomSqlDatetimeCastAsArg(MutliShardTestCaseColumn column) => string.Format("CAST(SYSDATETIME() AS {0})", column.SqlServerDatabaseEngineType);

    /// <summary>
    /// Helper that produces a random datetimeoffset value.
    /// </summary>
    /// <returns>The tsql to generate the desired value.</returns>
    private static string GetRandomSqlDatetimeoffsetValue() => "SYSDATETIMEOFFSET()";
    ...
    {
        var randomSmallMoneyValue = s_random.NextDouble() * 214748.3647;
    ...
    {
        var randomDouble = s_random.NextDouble() * double.MaxValue;
    ...
    private static string GetRandomSqlDecimalValue(MutliShardTestCaseColumn column)
    {
        ...
        //
        var theValue = s_random.NextDouble() * decimal.ToDouble(decimal.MaxValue);
    ...
    {
        var theValue = s_random.NextDouble() * SqlDouble.MaxValue.Value;
    ...
    {
        var theValue = s_random.NextDouble() * SqlMoney.MaxValue.ToDouble();
        return GetSpecificDoubleCastAsArg(theValue, column);
    }
    ...
    {
        var theValue = s_random.NextDouble() * SqlSingle.MaxValue.Value;
        return GetSpecificDoubleCastAsArg(theValue, column);
    }
    ...
    private static string GetSpecificDoubleCastAsArg(double theValue, MutliShardTestCaseColumn column) => string.Format("CAST({0} AS {1})", theValue, column.SqlServerDatabaseEngineType);

    /// <summary>
    /// Helper to generate a random int.
    /// </summary>
    /// <returns>The random int.</returns>
    private static string GetRandomSqlIntValue() => s_random.Next().ToString();
    ...
    private static string GetRandomSqlNCharValue(int length) => string.Format("N'{0}'", GetRandomString(length));

    /// <summary>
    /// Helper to generate a random value drawn from the TinyInt domain.
    /// </summary>
    /// <returns>The tsql to generate the desired value.</returns>
    private static string GetRandomSqlTinyIntValue() => s_random.Next(byte.MinValue, byte.MaxValue).ToString();

    /// <summary>
    /// Helper to generate a new guid.
    /// </summary>
    /// <returns>Tsql to produce the guid.</returns>
    private static string GetRandomSqlUniqueIdentifierValue() => "NEWID()";
    ...
    private static string GetRandomString(int length)
    {
        ...
        {
            var nextChar = Convert.ToChar(Convert.ToInt32(Math.Floor((26 * s_random.NextDouble()) + 65)));
            _ = builder.Append(nextChar);
        }
    ...
    /// <returns></returns>
    private static bool IsTimestampField(SqlDbType curFieldType) => SqlDbType.Timestamp == curFieldType;
