    ...
    public class MultiShardQueryE2ETests
    {
            ...
            using (var conn = new MultiShardConnection(_shardMap.GetShards(), MultiShardTestUtils.ShardConnectionString))
            {
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    using (var sdr = cmd.ExecuteReader())
                    {
                        ...
                        _ = AssertExtensions.AssertThrows<MultiShardDataReaderClosedException>(() => sdr.Read());
                    }
                }
            }
        }
        ...
        public void TestQueryShardsInvalidConnectionSync()
        {
            ...
            try
            {
                using (var conn = new MultiShardConnection(_shardMap.GetShards(), MultiShardTestUtils.ShardConnectionString))
                {
                    conn.GetShardConnections().Add(new Tuple<ShardLocation, DbConnection>(badShard, badConn));
                    using (var cmd = conn.CreateCommand())
                    {
...
drop type dbo.PageViewTableType
        ...
        {
            var cancelledShards = new List<ShardLocation>();
            ...
            using (var cmd = _shardConnection.CreateCommand())
            {
                ...
                _ = AssertExtensions.WaitAndAssertThrows<TaskCanceledException>(cmdTask);
        ...
        private List<Tuple<ShardLocation, DbConnection>> GetProxyShardConnections(ProxyServer proxy)
        {
            ...
            var rVal = new List<Tuple<ShardLocation, DbConnection>>();
            foreach (var shard in _shardMap.GetShards())
            {
                ...
                var curTuple = new Tuple<ShardLocation, DbConnection>(curLoc, curConn);
                rVal.Add(curTuple);
            }
