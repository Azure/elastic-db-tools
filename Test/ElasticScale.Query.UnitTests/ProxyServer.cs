...
public class ProxyServer : IDisposable
{
    ...
    private static readonly string s_logHeader = "======== ProxyServer Log Start ========\n";
    private static readonly string s_logTrailer = "======== ProxyServer Log End ========\n";

    /// <summary>
    /// Gets the event log for the proxy server
    /// </summary>
    internal StringBuilder EventLog { get; } = new StringBuilder(s_logHeader, 1000);

    /// <summary>
    /// SleepResetEvent to cancel proxy server sleep delay
    /// </summary>
    internal ManualResetEventSlim SleepResetEvent = new();
    ...
    /// </summary>
    internal bool StopRequested { get => _stopRequested; set => _stopRequested = value; }
            ...
            while (!StopRequested)
            {
                if (ListenerSocket.Pending())
                {
                    try
                    {
                        ...
                        var newConnection = ListenerSocket.AcceptTcpClient();

                        //Log("Connection accepted");

                        // Start a new connection
                        var proxyConnection = new ProxyServerConnection(newConnection, this);
    ...
    internal void Log(string text, params object[] args)
    {
        lock (EventLog)
        {
            _ = EventLog.AppendFormat("[{0:O}]: ", DateTime.Now);
            _ = EventLog.AppendFormat(text, args);
            _ = EventLog.AppendLine();
        }
    }
    ...
    public string GetServerEventLog()
    {
        lock (EventLog)
        {
            _ = EventLog.Append(s_logTrailer);
...
internal class ProxyServerConnection
{
    ...
    public void Start()
    {
        ...
        var incomingIPEndPoint = IncomingConnection.Client.RemoteEndPoint as IPEndPoint;
    ...
    private void _ProcessorHandler()
    {
        try
        {
            ...
            var outStream = OutgoingConnection.GetStream();
            var inStream = IncomingConnection.GetStream();
    ...
    private void CopyData(NetworkStream readStream, string readStreamName, NetworkStream writeStream, string writeStreamName, bool simulateDelay)
    {
        ...
        while (readStream.DataAvailable)
        {
            ...
            var numBytes = readStream.Read(buffer, 0, buffer.Length);
        ...
        foreach (var b in outBytes)
        {
            ...
            if (simulateDelay && !Server.SleepResetEvent.IsSet)
            {
                ...
                _ = Server.SleepResetEvent.Wait(Server.SimulatedPacketDelay);
            }
