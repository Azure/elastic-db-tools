    ...
    public class ScenarioTests
    {
        ...
        public static void ScenarioTestsInitialize(TestContext testContext)
        {
            ...
            using (SqlConnection conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.DropDatabaseQuery, ScenarioTests.PerTenantDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.CreateDatabaseQuery, ScenarioTests.PerTenantDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                ...
                {
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.DropDatabaseQuery, ScenarioTests.MultiTenantDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.CreateDatabaseQuery, ScenarioTests.MultiTenantDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
        ...
        public void BasicScenarioListShardMaps()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ScenarioTests.PerTenantDBs.Length; i++)
                {
                    ...
                    Assert.IsTrue(result.Shard.Location.Database == ScenarioTests.PerTenantDBs[i != 2 ? i : 0]);
                }
            ...
            try
            {
                ...
                for (int i = 0; i < ScenarioTests.MultiTenantDBs.Length; i++)
                {
                    ...
                    Shard s = multiTenantShardMap.CreateShard(
                        new ShardLocation(
                            Globals.ShardMapManagerTestsDatasourceName,
                            ScenarioTests.MultiTenantDBs[i]));
                ...
                for (int i = 0; i < ScenarioTests.MultiTenantDBs.Length; i++)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        Assert.IsTrue(result.Shard.Location.Database == ScenarioTests.MultiTenantDBs[0]);
                    }
                    else
                        if (i < 4)
                        {
                            Assert.IsTrue(result.Shard.Location.Database == ScenarioTests.MultiTenantDBs[i + 1]);
                        }
                        else
                        {
                            Assert.IsTrue(result.Shard.Location.Database == ScenarioTests.MultiTenantDBs[2]);
                        }
                }
                ...
                Shard targetShard = multiTenantShardMap.GetShard(new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ScenarioTests.MultiTenantDBs[0]));
