...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement.UnitTests
{
    ...
    public class ShardMapTests
    {
        ...
        /// </summary>
        private static string DefaultShardMapName = "Customers_default";
        ...
        public static void ShardMapTestsInitialize(TestContext testContext)
        {
            ...
            using (SqlConnection conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
            {
                ...
                for (int i = 0; i < ShardMapTests.ShardedDBs.Length; i++)
                {
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.DropDatabaseQuery, ShardMapTests.ShardedDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.CreateDatabaseQuery, ShardMapTests.ShardedDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
            ...
            ShardMap sm = smm.CreateListShardMap<int>(ShardMapTests.DefaultShardMapName);

            Assert.IsNotNull(sm);

            Assert.AreEqual(ShardMapTests.DefaultShardMapName, sm.Name);
        }
        ...
        public void CreateShardDuplicate()
        {
            ...
            ShardLocation sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapTests.ShardedDBs[0]);
