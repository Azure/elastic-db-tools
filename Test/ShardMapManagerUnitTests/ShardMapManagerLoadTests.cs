...
namespace Microsoft.Azure.SqlDatabase.ElasticScale.ShardManagement.UnitTests
{
    ...
    public class ShardMapManagerLoadTests
    {
        ...
        /// List shard map name.
        /// </summary>
        private static string ListShardMapName = "Customers_list";

        /// <summary>
        /// Range shard map name.
        /// </summary>
        private static string RangeShardMapName = "Customers_range";
        ...
        /// </summary>
        private static string[] DeadlockDetectionCleanupQueries = new[]
        {
            "use msdb",
        ...
        /// These will not work against Azure SQL DB, code just catches and ignores SqlException for these queries.
        /// </summary>
        private static string[] DeadlockDetectionSetupQueries = new[]
        {
            "use msdb",
        ...
        /// </summary>
        private static string DeadlockDetectionQuery = "select CAST(message_body AS XML) from msdb..DeadlockQueue";

        /// <summary>
        /// Number of shards added to both list and range shard maps.
        /// </summary>
        const int InitialShardCount = 6;
        
        /// <summary>
        /// Lowest point on Integer range that can be mapped by unit tests.
        /// </summary>
        const int MinMappingPoint = -2000;

        /// <summary>
        /// Highest point on Integer range that can be mapped by unit tests.
        /// </summary>
        const int MaxMappingPoint = 2000;

        /// <summary>
        /// Maximum size of a single range mapping.
        /// </summary>
        const int MaxRangeMappingSize = 10;
        ...
        /// </summary>
        static RetryPolicy<SqlAzureTransientErrorDetectionStrategy> retryPolicy;
        ...
        public static void ShardMapManagerLoadTestsInitialize(TestContext testContext)
        {
            ...
            using (SqlConnection conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
            {
                ...
                for (int i = 0; i < ShardMapManagerLoadTests.ShardedDBs.Length; i++)
                {
                    using (SqlCommand cmd = new SqlCommand(
                        string.Format(Globals.CreateDatabaseQuery, ShardMapManagerLoadTests.ShardedDBs[i]),
                        conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                ...
                foreach (string q in DeadlockDetectionCleanupQueries)
                {
                ...
                foreach (string q in DeadlockDetectionSetupQueries)
                {
                    using (SqlCommand cmd = new SqlCommand(q, conn))
                    {
                        try
            ...
            ListShardMap<int> lsm = smm.CreateListShardMap<int>(ShardMapManagerLoadTests.ListShardMapName);
            Assert.IsNotNull(lsm);

            // Create range shard map.
            RangeShardMap<int> rsm = smm.CreateRangeShardMap<int>(ShardMapManagerLoadTests.RangeShardMapName);
            ...
            for (int i=0; i<ShardMapManagerLoadTests.InitialShardCount; i++)
            {
                ShardCreationInfo si = new ShardCreationInfo(
                    new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, ShardMapManagerLoadTests.ShardedDBs[i]),
                    ShardStatus.Online);
            ...
            retryPolicy = new RetryPolicy<SqlAzureTransientErrorDetectionStrategy>(
        ...
        public static void ShardMapManagerLoadTestsCleanup()
        {
            ...
            using (SqlConnection conn = new SqlConnection(Globals.ShardMapManagerTestConnectionString))
            {
                ...
                {
                    cmd.CommandText = DeadlockDetectionQuery;
                ...
                if (!deadlocksDetected && !inconsistencyDetected)
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand(
                            string.Format(Globals.DropDatabaseQuery, ShardMapManagerLoadTests.ShardedDBs[i]),
                            conn))
                        {
                            cmd.ExecuteNonQuery();
                        }
        ...
        static void ValidateImpl(ShardMap sm, int key)
        {
            try
            {
        ...
        PointMapping<int> GetRandomPointMapping(ListShardMap<int> lsm)
        {
            ...
            int index = r.Next(allMappings.Count);
        ...
        RangeMapping<int> GetRandomRangeMapping(RangeShardMap<int> rsm, int minimumRangeSize = 1)
        {
            ...
            return filteredList[r.Next(filteredList.Count)];
        ...
        private Shard GetRandomOnlineShardFromShardMap(ShardMap sm)
        {
            ...
            if (shardList.Count > 0)
                return shardList[r.Next(shardList.Count)];
            else
                return null;
        }
        ...
        private void AddShardToShardMap(ShardMap sm)
        {
            ...
            List<string> availableLocationList = (from dbName in ShardMapManagerLoadTests.ShardedDBs
            ...
            {
                ShardLocation sl = new ShardLocation(Globals.ShardMapManagerTestsDatasourceName, availableLocationList[r.Next(availableLocationList.Count)]);
        ...
        public void LoadTestAddPointMapping()
        {
            try
            {
                ...
                ListShardMap<int> lsm = smm.GetListShardMap<int>(ShardMapManagerLoadTests.ListShardMapName);
                ...
                do
                {
                   ...
                   // Create a random integer key for a new mapping and verify that its not already present in this shard map.
                    int key = r.Next(MinMappingPoint, MaxMappingPoint);
        ...
        public void LoadTestRemoveShardFromListShardMap()
        {
            try
            {
                ...
                {
                    offlineShard = existingShards[r.Next(existingShards.Count)];
        ...
        public void LoadTestAddRangeMapping()
        {
            try
            {
                ...
                RangeShardMap<int> rsm = smm.GetRangeShardMap<int>(ShardMapManagerLoadTests.RangeShardMapName);
                ...
                do
                {
                    ...
                    int minKey = r.Next(MinMappingPoint, MaxMappingPoint);
                    int maxKey = minKey + r.Next(1, MaxRangeMappingSize);
        ...
        public void LoadTestRangeMappingDDR()
        {
            try
            {
                ...
                if (r1 != null)
                {
                    int keyToValidate = r.Next((int)(r1.Range.Low.Value), (int)(r1.Range.High.Value));
        ...
        public void LoadTestSplitRangeWithLock()
        {
            try
            {
                ...
                if (r1 != null)
                {
                    int splitPoint = r.Next((int)(r1.Range.Low.Value)+1, (int)(r1.Range.High.Value)-1);
        ...
        public void LoadTestKillLSMConnections()
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    databaseName = sl[r.Next(sl.Count)].Database;
                }
                while (false);
            }
            catch (ShardManagementException sme)
