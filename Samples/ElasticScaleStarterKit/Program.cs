...
internal class Program
{
    ...
    private static void PrintShardMapState()
    {
        ...
        if (allShards.Any())
        {
            ...
            foreach (var shard in shardMap.GetShards().OrderBy(s => s.Location.Database))
            {
                ...
                {
                    var mappingsString = string.Join(", ", mappingsOnThisShard.Select(m => m.Value));
    ...
    private static bool GetMenuChoiceAndExecute()
    {
        while (true)
        {
            var inputValue = ConsoleUtils.ReadIntegerInput("Enter an option [1-6] and press ENTER: ");
    ...
    private static void CreateShardMapManagerAndShard()
    {
        ...
        // Create shard map
        var shardMap = ShardManagementUtils.CreateOrGetRangeShardMap<int>(
    ...
    private static RangeShardMap<int> TryGetShardMap()
    {
        ...
        var mapExists = s_shardMapManager.TryGetRangeShardMap(Configuration.ShardMapName, out
