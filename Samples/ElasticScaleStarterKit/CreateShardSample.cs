...
internal class CreateShardSample
{
    ...
    public static void CreateShard(RangeShardMap<int> shardMap, Range<int> rangeForNewShard)
    {
        ...
        var shard = CreateOrGetEmptyShard(shardMap);

        // Create a mapping to that shard.
        var mappingForNewShard = shardMap.CreateRangeMapping(rangeForNewShard, shard);
    ...
    private static Shard CreateOrGetEmptyShard(RangeShardMap<int> shardMap)
    {
        ...
        var shard = FindEmptyShard(shardMap);
        if (shard == null)
        {
            // No empty shard exists, so create one

            // Choose the shard name
            var databaseName = string.Format(ShardNameFormat, shardMap.GetShards().Count());
            ...
            var shardLocation = new ShardLocation(Configuration.ShardMapManagerServerName, databaseName);
